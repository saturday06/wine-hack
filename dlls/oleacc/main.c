...
HRESULT WINAPI ObjectFromLresult( LRESULT result, REFIID riid, WPARAM wParam, void **ppObject )
{
    ...
    if (result_handle != S_OK) {
        e = GetLastError();
        TRACE("CoUnmarshalInterface e=%d hr=%d\n", e, result_handle);
        goto clean_up;
    }
...
LRESULT WINAPI LresultFromObject( REFIID riid, WPARAM wParam, LPUNKNOWN pAcc )
{
    ...
    if (!shared_variable_handle) {
        TRACE("invalid shared_variable_handle\n");
    ...
    if (wait_result != WAIT_OBJECT_0) {
        e = GetLastError();
        TRACE("WaitForSingleObject wait_result=%d e=%d\n", wait_result, e);
        return_value = E_OUTOFMEMORY;
        goto clean_up;
    }
    ...
    if (shared_variable == NULL) {
        e = GetLastError();
        TRACE("MapViewOfFile failure e=%d\n", e);
        return_value = E_OUTOFMEMORY;
        goto clean_up;
    }
    ...
    if (result_handle != S_OK) {
        DWORD e = GetLastError();
        TRACE("CoMarshalInterface e e=%d hr=%d\n", e, result_handle);
        goto clean_up;
    }
    ...
    switch (fdwReason)
    {
        case DLL_PROCESS_ATTACH: {
            ...
            if (!shared_variable_mutex_handle) {
                DWORD e = GetLastError();
                TRACE("SELENIUM CreateMutex failure e=%d\n", e);
                break;
            }
            ...
            if (!shared_variable_handle) {
                DWORD e = GetLastError();
                TRACE("SELENIUM CreateFileMapping failure e=%d\n", e);
                break;
            }
            break;
        }
